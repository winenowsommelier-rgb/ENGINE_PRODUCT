#!/usr/bin/env python3
"""
Phase D2 — Description enrichment for products with sales signal but no description.

Uses Sonnet 4.6 (quality matters — these are actively selling). Excludes:
- Non-beverage SKU prefixes (ABA/AWC/CIG/GBE/GDC/GLQ/GWN/WEV)
- Mineral Water, Accessories, Events, Glassware, Cigar classifications
- NNA prefix (Monin syrups — sommelier prompt is wrong for cocktail syrups)

Usage:
    # 5-SKU canary (ALWAYS run first per Rule 10):
    .venv/bin/python scripts/phase_d2_enrich_has_sales.py --limit 5 --dry-run
    .venv/bin/python scripts/phase_d2_enrich_has_sales.py --limit 5

    # Full run (after canary sign-off):
    .venv/bin/python scripts/phase_d2_enrich_has_sales.py

    # Resume after partial run:
    .venv/bin/python scripts/phase_d2_enrich_has_sales.py --skip-done
"""
import argparse
import json
import shutil
...
ENRICHMENT_SOURCE = "phase_d2_sonnet_has_sales"
...
2. **TECHNIQUE-FORWARD** — quote what's DONE to the product:
   ...
   - Liqueur/Bitters/Absinth: base spirit, botanical sources, production method, maceration vs distillation
   - Mezcal: agave variety, roasting method (earthen pit vs above-ground), distillation vessel (clay vs copper)
...
def fix_unescaped_quotes(text: str) -> str:
    """Escape unescaped double-quotes inside JSON string values (state-machine)."""
    out = []
    i = 0
    n = len(text)
    in_str = False
...
def main(argv=None):
    ...
    targets = [dict(r) for r in conn.execute("""
        ...
        WHERE COALESCE(is_active,1)=1
          ...
          AND sku NOT LIKE 'NNA%'
        ORDER BY COALESCE(popularity_revenue_window,0) DESC, has_recent_sales DESC, sku
    """)]
    ...
    est_cost = len(targets) * (1400 * COST_IN + 700 * COST_OUT)
    ...
    if not args.dry_run and not args.no_backup:
        ...
        bak = args.db.with_suffix(args.db.suffix + f".bak-phase-d2-{ts}")
        shutil.copy2(args.db, bak)
        print(f"Backup: {bak}\n")

    ts = datetime.now().strftime("%Y%m%d-%H%M%S")
    sidecar_path = REPO_ROOT / f"data/phase_d2_results-{ts}.jsonl"
    ...
    def work(i: int, row: dict):
        ...
        if r["status"] in ("ok", "ok_fixed"):
            ...
            print(f"  [{i+1:>3}/{len(targets)}] {row['sku']} {r['status']}  ${r['cost_usd']:.4f}  \"{short}\"")
        else:
            if r["status"].startswith("parse"):
                stats["parse_error"] += 1
            else:
                stats["api_error"] += 1
            print(f"  [{i+1:>3}/{len(targets)}] {row['sku']} {r['status']}", file=sys.stderr)
