...
export function Dashboard() {
  ...
  async function handleSaveToDatabase() {
    if (!uploadedDataset) {
      setSaveState('error');
      setSaveMessage('Upload a Magento CSV first so the app has staged rows to save.');
      return;
    }

    setSaveState('saving');
    setSaveMessage('Saving staged rows to Supabase...');

    try {
      const result = await persistImportToSupabase({
        sourceFilename: uploadedDataset.sourceFile,
        batchResult: activeBatchResult
      });

      setSaveState('success');
      setSaveMessage(
        `Saved ${result.savedProducts} ready products to Supabase. Import run ${result.importRunId} recorded ${result.stagedRows} staged rows and ${result.blockedRows} blocked rows.`
      );
    } catch (error) {
      setSaveState('error');
      setSaveMessage(
        `${error instanceof Error ? error.message : 'Unable to save to Supabase.'} Make sure supabase/schema.sql is applied and insert policies allow the publishable key to write to import_runs, import_run_rows, products, and flavor_profile.`
      );
    }
  }
            ...
            <div className="mt-6 rounded-3xl border border-dashed border-violet-400/40 bg-violet-500/10 p-5">
              ...
              <div className="mt-4 flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
                <p className="text-sm text-slate-300">
                  After validation, save the cleaned rows into Supabase so the item library and product matrix can continue from the database instead of only local browser state.
                </p>
                <button
                  type="button"
                  onClick={handleSaveToDatabase}
                  disabled={!uploadedDataset || saveState === 'saving'}
                  className={`rounded-full px-5 py-3 text-sm font-medium transition ${
                    !uploadedDataset || saveState === 'saving'
                      ? 'cursor-not-allowed border border-white/10 bg-white/5 text-slate-500'
                      : 'border border-emerald-300/40 bg-emerald-500/15 text-emerald-100 hover:border-emerald-200/60 hover:text-white'
                  }`}
                >
                  {saveState === 'saving' ? 'Saving to Supabase…' : 'Save ready rows to database'}
                </button>
              </div>
              {saveMessage && (
                <div
