...
- **Grape links**: `link_grape` for ONLY the grapes that exist as entities from Plan 1: `cabernet-sauvignon`, `merlot`, `cabernet-franc`, `sauvignon-blanc`, `semillon` → Bordeaux region. **Do NOT link `petit-verdot`** — it is NOT a loaded entity, and `_helpers.link_grape` RAISES `ValueError` on a missing grape (it does not skip). If you want Petit Verdot represented, first `ingest.upsert_entity(conn, "grape_variety", "Petit Verdot", "petit-verdot")` with its own cited context, THEN link it — but this is optional; the 5 confirmed grapes are sufficient for this task.
