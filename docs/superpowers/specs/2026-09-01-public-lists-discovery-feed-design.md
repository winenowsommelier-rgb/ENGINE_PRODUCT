...
**Overall end-of-feed / `nextCursor` rule:** `getDiscoverFeed` returns `{ pins: PublicPinRow[], nextCursor: DiscoverCursor | null }`. The top-level `nextCursor` is `null` **only when all four per-source sub-queries returned zero rows on this call** (i.e. every source is genuinely exhausted at once) — NOT merely when the merged, truncated page has fewer than `limit` total rows, since a short page can still leave one or more sources with unfetched rows sitting behind an as-yet-unadvanced cursor (their query for this page just returned fewer than their share, not zero). Concretely: after running all four sub-queries and merging/truncating, if the sum of raw rows returned by the four sub-queries (before truncation) is zero, return `nextCursor: null`; otherwise return the populated `DiscoverCursor` (with each source's key present or carried-over per the rule above) so `PinGrid` keeps calling `loadMorePinsAction`. This is the one sentence `PinGrid`'s infinite-scroll stop condition (below) depends on, and is stated explicitly here rather than left for the plan to infer, since an implementer inferring "fewer than `limit` total = stop" (the V1 single-source rule, naively carried over) would stop the feed early whenever the four sources' combined contribution happens to undershoot `limit` on one page even though more rows exist.
