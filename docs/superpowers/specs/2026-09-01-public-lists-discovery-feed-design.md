...
- **Filter:** rows are grouped by `SKU`; a SKU qualifies for `requested_items` only if it appears in **2 or more distinct tickets** (empirically, ~180 of ~650 distinct SKUs meet this bar over the sheet's ~3-week history as of 2026-09-04 — a large enough pool to matter, not so large it stops being a signal) **and** its most recent `Stock Status` in the sheet indicates in-stock (not `0`, not "Discontinue", not the Thai out-of-stock equivalents). A SKU that later goes out of stock drops out of `requested_items` on the next sync, same as it would from `getProductBySku` resolving to nothing (Rule 6 invariant applies here too).
...
- **Failure mode:** if the sheet URL is unreachable or returns malformed CSV, the sync job logs and skips that run, leaving `requested_items` at its last-synced state — never blocks or degrades the `/discover` page itself, which reads only from `requested_items`, not the sheet directly.
...
**Feed assembly (`getDiscoverFeed`):** combines the four sources into one page, sorted by their respective timestamp (real for Pins/Staff Picks, synthetic for Trending/Requested — see Multi-source feed above), then paginated with the same keyset-cursor mechanics as the original `getPublicPinsFeed` design (Data access above) — the cursor tuple becomes `(sort_ts, id)` where `id` is a stable per-source-row identifier (real `list_items.id` for Pins, `curated_pins.id` for Staff Picks, and a deterministic synthetic id — e.g. a UUID derived from `sku` — for Trending/Requested, so the tie-breaker stays gap-free even for sources with no natural row id). This is an implementation detail to work out precisely in the plan, not fully specified here — flagging the shape of the problem (four heterogeneous sources merged into one keyset-paginated stream) so the plan doesn't discover it fresh.
...
- `SaveToListButton` — present on **all four sources** (this was an explicit product decision — see Multi-source feed above), passed `sku`, `isLoggedIn`, `userLists`, identical props contract to its existing PDP/ProductCard usage regardless of source, no changes to that component required.
