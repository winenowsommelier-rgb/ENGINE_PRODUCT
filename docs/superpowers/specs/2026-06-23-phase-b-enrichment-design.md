...
- **Run 1 (this spec): sharpen the Product Finder.** Fill ONLY the attribute columns the
  finder actually scores AND that are genuinely empty for non-wine drinkables.
...
**Re-scoped Run 1 fields:** `variety` + `body` — both finder-scored and shop-filterable.
...
**Measured size (queried 2026-06-23):** **794 rows** to call (781 need variety, 505 need
body; one call fills both). Category-group resolved via `sku_taxonomy.resolve()` (Rule 12 —
SKU is source of truth, NOT classification).

---

## 4. Fields, scales & validation

### 4.1 `body` — UNIVERSAL Light→Full scale (all categories)

The finder's body ladder (`scoring.ts` `bodyLadderDistance` → `taste-adapter.ts`
`normalizeScale('body')`) expects the **wine scale**:

```
["Light", "Medium-Light", "Medium", "Medium-Full", "Full"]
```

So body MUST be written in this scale for ALL categories, or the finder ladder mis-ranks.
The category-specific `body_texture`/`body_umami` axes in `category_axes.py` are for the
DISPLAY TasteWheel, a different consumer — do NOT use them for the flat `body` column.
The prompt instructs the model to map a spirit's weight onto this scale
...
```
select rows (§3) ── products.db
   │ per row: name, category_type, existing flavor_tags/variety as context
   ▼
scripts/enrich_phase_b.py        (forked from scripts/phase_d1_enrich_critic_scored.py)
   • schema_for_type(category_type)  → which fields + scales
   • Haiku 4.5, constrained JSON out  → {variety, body}
   • validate each against scale; drop off-scale (write null)
   • --limit N / --dry-run canary; threaded; DB backup on --apply
   ▼  writes FIRST to:
enrichment_cache (Supabase) + local sidecar     sku → {variety, body, source, model, ts}
   │  SEPARATE, idempotent, NULL-only merge step
   ▼
products.db (UPDATE ... WHERE col IS NULL) ─► refresh_live_export.py ─► live_products_export.json
   ▼
VERIFY-SHIPPED (Rule 1): count variety/body in the JSON == DB; spot-check finder surfaces them
```
...
## 6. Reused skeleton (Rule 11)

Fork `scripts/phase_d1_enrich_critic_scored.py` — it already provides: `--limit`/`--dry-run`
canary, Haiku 4.5 + per-token cost constants, DB backup on apply, `ThreadPoolExecutor`,
cache sidecar, `--skip-done` resume, and a factual-discipline system prompt. New surface is
small: the constrained-output prompt, the enum validator, and `schema_for_type`. Reuses the
Supabase `enrichment_cache` and `backfill_from_cache.py`-style merge.
...
- Unit: `schema_for_type` returns correct fields/scales per `category_type`; variety validator
  drops off-vocab; body validator accepts only the Light→Full scale.
...
- Canary acceptance: 5 real SKUs produce in-vocab, plausible values (manual eyeball).
