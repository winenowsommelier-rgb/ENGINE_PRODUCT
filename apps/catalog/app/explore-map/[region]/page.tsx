...
export function generateMetadata({ params }: { params: { region: string } }): Metadata {
  ...
  // Case-insensitive trim match: product.region may differ in casing from MapRegion.name
  ...
  return {
    ...
    alternates: { canonical: `https://wnlq9-catalog.vercel.app/explore-map/${r.slug}` },
...
export default function RegionPage({ params }: { params: { region: string } }) {
  ...
  // Case-insensitive trim match to guard against minor casing discrepancies
  ...
  // Top 5 for CollectionPage.hasPart
